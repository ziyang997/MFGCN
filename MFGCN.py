...
class MFGCN:
    def __init__(self, data, pretrain_data=None):
        ...
        self.user_features_pre = self.get_user_embedding()
        self.item_features_pre = self.get_user_embedding()
    ...
    def forward(self):
        ...
        return tf.reduce_mean(user_features, axis=0), tf.reduce_mean(item_features, axis=0)

    def loss_net(self):
        #egular_loss = [tf.nn.l2_loss(weight) for weight in self.weights.values()]
        #emb_loss = tf.reduce_mean(regular_loss) * decay
        user_features_pre = tf.nn.embedding_lookup(self.user_features_pre, self.users)
        item_pos_features_pre = tf.nn.embedding_lookup(self.item_features_pre, self.pos_items)
        item_neg_features_pre = tf.nn.embedding_lookup(self.item_features_pre, self.neg_items)
        # user_features = tf.nn.embedding_lookup(user_embedding,self.rating['UserID'])
        regular_loss = tf.nn.l2_loss(user_features_pre) + tf.nn.l2_loss(item_pos_features_pre) + tf.nn.l2_loss(item_neg_features_pre)
        emb_loss = regular_loss * decay / BATCH_SIZE
        ...
        mf_loss = tf.reduce_mean(tf.negative(tf.log(tf.sigmoid(pos_scores)) + tf.log(1-tf.nn.sigmoid(neg_scores))))
...
class sample_thread(threading.Thread):
    ...
    def run(self):
        with tf.device(cpus[0]):
            self.data = data_generator.sample_train()
...
if __name__=='__main__':
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for idx in range(n_batch):
            ...
            _, batch_loss, batch_mf_loss, batch_emb_loss= train_cur.data
