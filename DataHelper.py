...
class Data:
    def __init__(self, batch_size=512, name='ml-1m'):
        ...
        # Static Profile
        self.ftr_size = {}
        self.n_items = self.ftr_size['Movie']
        self.n_users = self.ftr_size['User']
    ...
    def getMovieInfo(self):
        if self.dataName == "ml-1m":
            ...
            rated_movie_id = set(sorted(rated_movie_id))
            ...
            genres2int = {val: idx for idx, val in enumerate(genres)}
    ...
    def negative_pool(self):
        for u in self.train_set.keys():
            ...
            pools = [np.random.choice(neg_items, 4, replace=False)]
            self.neg_pools[u] = pools
    def sample_train_old(self):
        n_users = self.ftr_size['User']
    ...
    def sample_train(self):
        if self.batch_size <= self.n_users:
            users = np.random.choice(np.arange(self.n_users), self.batch_size, replace=False)
        else:
            users = np.arange(self.n_users)
        pos_items, neg_items = [], []
        for u in users:
            pos_items.append(np.random.choice(self.train_set[u], 1)[0])
            neg_items.append(np.random.choice(self.neg_pools[u], 1)[0])
        return users, pos_items, neg_items

    def sample_test_old(self):
    ...
    def sample_test(self):
        ...
        for u in users:
            pos_items.append(np.random.choice(self.test_set[u], 1)[0])
            neg_items.append(np.random.choice(list(set(self.neg_pools[u]) - set(self.test_set[u])), 1)[0])
        return users, pos_items, neg_items

def split(full_list):
        sublist_1 = full_list[:-1]
        sublist_2 = full_list[-1:]
